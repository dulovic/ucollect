...
static void engine_init_evaluators(struct context *ctx, struct engine *en) {
	...
	struct evaluator_context ev_ctx = {
        ...
        en->learn_profile_evaluator_offsets = mem_pool_alloc(ctx->permanent_pool, en->timeslot_cnt * sizeof(size_t));
        en->detect_profile_evaluator_offsets = mem_pool_alloc(ctx->permanent_pool, en->timeslot_cnt * sizeof(size_t));
	...
	for (i = 0; i < en->evaluator_cnt; i++) {
		...
		en->learn_profile_evaluator_offsets[i] = en->learn_profile_size;
                en->detect_profile_evaluator_offsets[i] = en->detect_profile_size;
...
void nasrac() {
	...
	// Get host profiles
	uint8_t *learn_profiles = engine_lookup_host_learn_profiles(en, key, key_size);
	uint8_t *detect_profiles = NULL;
	if (en->mode == DETECTION) {
		detect_profiles = engine_lookup_host_detect_profiles(en, key, key_size);
	}
	...
	for (i_sm = 0; i_sm < en->statemachine_cnt; i_sm++) {
		...
		while (conv = sm->get_next_finished_conv_callback(&sm_ctx)) {
			...
			struct *learn_profile learning = (struct learn_profile *) learn_profiles + en->learn_profile_evaluator_offsets[]
		}
	}
